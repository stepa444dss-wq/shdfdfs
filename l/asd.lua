...
-- Library init
    getgenv().Library = {
        ...
        KeybindHolders = {}; -- СЃРїРёСЃРѕРє РІСЃРµС… ModeHolder'РѕРІ РєРµР№Р±РёРЅРґРѕРІ РґР»СЏ СЃРєСЂС‹С‚РёСЏ РїСЂРё ToggleMenu
    ...
    local Fonts = {}; do
        function RegisterFont(Name, Weight, Style, Asset)
            if not Asset.Font then return nil end
            
            if not isfile(Asset.Id) then
                pcall(function() writefile(Asset.Id, Asset.Font) end)
            end
            
            if not isfile(Asset.Id) then return nil end
        ...
        local ok = pcall(function()
            fontData = game:HttpGet("https://github.com/i77lhm/storage/raw/refs/heads/main/fonts/fs-tahoma-8px.ttf")
        end)
        if not ok or not fontData or #fontData == 0 then
            warn("font wont load idk why")
        end
    ...
    -- Misc functions
        ...
        function Library:UpdateConfigList() 
            ...
            for _,file in listfiles(Library.Directory .. "/configs") do
                ...
                -- РёРіРЅРѕСЂРёСЂСѓРµРј autoload.txt Рё Р»СЋР±С‹Рµ РЅРµ-.cfg С„Р°Р№Р»С‹
                if Name == "autoload" or Name == "autoload.txt" or file:find("autoload") then
                    continue
                end
                if not file:find("%.cfg$") then
                    continue
                end
                List[#List + 1] = Name
            end
        ...
        function Library:Keypicker(properties) 
                            ...
                            while true do 
                                ...
                                if Type and Type ~= "None" then 
                                    ...
                                    if Type == "Rainbow" then
                                        local hue = (tick() * (Flags[Cfg.Flag .. "_ANIMATION_SPEED"] / 25)) % 1
                                        Cfg.Set(hsv(hue, 1, 1), a)
            ...
            -- Р—Р°РєСЂС‹РІР°РµРј РїРѕРїР°Рї РїСЂРё РєР»РёРєРµ СЃРЅР°СЂСѓР¶Рё (С‚РѕР»СЊРєРѕ РґР»СЏ СЂРµР¶РёРјР° Keypicker, Р±РµР· Animation)
            -- Р’ СЂРµР¶РёРјРµ Animation Р°РЅР°Р»РѕРіРёС‡РЅС‹Р№ РѕР±СЂР°Р±РѕС‚С‡РёРє СѓР¶Рµ РґРѕР±Р°РІР»РµРЅ РІС‹С€Рµ СЃ СѓС‡С‘С‚РѕРј Dropdown'РѕРІ
            if Cfg.Mode ~= "Animation" then
                Library:Connection(InputService.InputBegan, function(input, game_event)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if Items.Colorpicker.Visible and not Library:Hovering({Items.ColorpickerObject, Items.Colorpicker}) then
                            Cfg.Open = false
                            Cfg.SetVisible(false)
                        end
                    end
        ...
        function Library:GetConfig()
            ...
            for Idx, Value in Flags do
                -- config_Name_list (РґСЂРѕРїРґР°СѓРЅ "Configs") Рё config_Name_text (РїРѕР»Рµ "Config Name:")
                -- СЌС‚Рѕ СЃР»СѓР¶РµР±РЅРѕРµ UI-СЃРѕСЃС‚РѕСЏРЅРёРµ РѕРєРЅР° СЃРѕС…СЂР°РЅРµРЅРёСЏ/Р·Р°РіСЂСѓР·РєРё, Р° РЅРµ РЅР°СЃС‚СЂРѕР№РєР° С„РёС‡Рё.
                -- РС… РЅРµР»СЊР·СЏ СЃРѕС…СЂР°РЅСЏС‚СЊ Р’ РЎРђРњ РєРѕРЅС„РёРі, РёРЅР°С‡Рµ Р·Р°РіСЂСѓР·РєР° РѕРґРЅРѕРіРѕ РєРѕРЅС„РёРіР° Р±СѓРґРµС‚
                -- Р·Р°С‚РёСЂР°С‚СЊ РёРјСЏ/РІС‹Р±РѕСЂ РґСЂСѓРіРѕРіРѕ РєРѕРЅС„РёРіР° С‡РµСЂРµР· РєР°СЃРєР°Рґ Dropdown -> Textbox callback'РѕРІ.
                if Idx == "config_Name_list" or Idx == "config_Name_text" then
                    continue
                end
                ...
                elseif type(Value) == "table" and Value["Mode"] ~= nil and Value["Active"] ~= nil and not Value["Transparency"] then
                    -- РєРµР№Р±РёРЅРґ: СЃРѕС…СЂР°РЅСЏРµРј Key РєР°Рє СЃС‚СЂРѕРєСѓ
                    local keyVal = Value["Key"] or Value["key"]
                    local keyStr
                    if type(keyVal) == "string" then
                        keyStr = keyVal
                    elseif keyVal ~= nil then
                        keyStr = tostring(keyVal)
                    else
                        keyStr = "NONE"
                    end
                    Config[Idx] = {Active = Value["Active"], Mode = Value["Mode"], key = keyStr}
                else
                    Config[Idx] = Value
                end
            end
        ...
        function Library:LoadConfig(JSON) 
            ...
            for Idx, Value in Config do                
                -- РќР° СЃР»СѓС‡Р°Р№ РµСЃР»Рё РіСЂСѓР·РёС‚СЃСЏ РЎРўРђР Р«Р™ .cfg С„Р°Р№Р» (СЃРѕС…СЂР°РЅС‘РЅРЅС‹Р№ РґРѕ СЌС‚РѕРіРѕ С„РёРєСЃР°),
                -- РІ РєРѕС‚РѕСЂРѕРј СЌС‚Рё СЃР»СѓР¶РµР±РЅС‹Рµ РїРѕР»СЏ РµС‰С‘ РїСЂРёСЃСѓС‚СЃС‚РІСѓСЋС‚ - РїСЂРѕРїСѓСЃРєР°РµРј РёС… Рё Р·РґРµСЃСЊ С‚РѕР¶Рµ.
                if Idx == "config_Name_list" or Idx == "config_Name_text" then 
                    continue 
                end
                ...
                if Function then 
                    ...
                    elseif type(Value) == "table" and (Value["Active"] ~= nil or Value["Mode"] ~= nil) and Value["key"] ~= nil then
                        -- РєРµР№Р±РёРЅРґ: РЅРѕСЂРјР°Р»РёР·СѓРµРј РїРѕР»СЏ РїРµСЂРµРґ РїРµСЂРµРґР°С‡РµР№ РІ Cfg.Set
                        Function({
                            Active = Value["Active"],
    ...
    -- Library element functions
        function Library:Window(properties)
            ...
            function Cfg.ToggleMenu(bool) 
                ...
                -- РїСЂРё СЃРєСЂС‹С‚РёРё РјРµРЅСЋ Р·Р°РєСЂС‹РІР°РµРј РІСЃРµ РѕС‚РєСЂС‹С‚С‹Рµ СЌР»РµРјРµРЅС‚С‹ (РєРѕР»РѕСЂРїРёРєРµСЂС‹, РґСЂРѕРїРґР°СѓРЅС‹) СЃ С„РµР№РґРѕРј
                if not bool then
                    Library:CloseElement()
                end

                -- С„РµР№РґРёРј РІСЃРµ РѕС‚РєСЂС‹С‚С‹Рµ Keybind ModeHolder'С‹ СЃРёРЅС…СЂРѕРЅРЅРѕ СЃ РјРµРЅСЋ
                if not bool then
                    for _, entry in Library.KeybindHolders do
                        local holder = entry.holder
                        entry.cfg.Open = false
                        -- С„РµР№РґРёРј РІСЃРµ РґРѕС‡РµСЂРЅРёРµ РѕР±СЉРµРєС‚С‹ ModeHolder'Р° СЃ С‚РµРј Р¶Рµ TweeningSpeed
                    ...
                    -- РїСЂРё РїРѕРєР°Р·Рµ РјРµРЅСЋ ModeHolder'С‹ РЅРµ С‚СЂРѕРіР°РµРј вЂ” РѕРЅРё РїРѕРєР°Р¶СѓС‚СЃСЏ РєРѕРіРґР° СЋР·РµСЂ РЅР°Р¶РјС‘С‚
                end
        ...
        function Library:Tab(properties)
            ...
            function Cfg.OpenTab() 
                ...
                -- Р·Р°РєСЂС‹РІР°РµРј РІСЃРµ РѕС‚РєСЂС‹С‚С‹Рµ Keybind ModeHolder'С‹ РїСЂРё СЃРјРµРЅРµ С‚Р°Р±Р°
                for _, entry in Library.KeybindHolders do
                    entry.cfg.Open = false
        ...
        function Library:Keybind(properties) 
            ...
            function Cfg.SetVisible(bool)
                ...
                -- СЂРµРіРёСЃС‚СЂРёСЂСѓРµРј/СѓР±РёСЂР°РµРј РёР· РіР»РѕР±Р°Р»СЊРЅРѕРіРѕ СЃРїРёСЃРєР° РѕС‚РєСЂС‹С‚С‹С… С…РѕР»РґРµСЂРѕРІ
                if bool then
        ...
        -- в”Ђв”Ђ Viewport РІРёРґР¶РµС‚ в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
        -- РЎРѕР·РґР°С‘С‚ РЅР°С‚РёРІРЅС‹Р№ Р±Р»РѕРє СЃ ViewportFrame РІРЅСѓС‚СЂРё СЃРµРєС†РёРё.
        -- properties:
        --   Height  (number)  вЂ” РІС‹СЃРѕС‚Р° Р±Р»РѕРєР° РІ РїРёРєСЃРµР»СЏС…, default 240
        --   Camera  (Camera)  вЂ” РЅРµРѕР±СЏР·Р°С‚РµР»СЊРЅРѕ, СЃРѕР·РґР°С‘С‚СЃСЏ Р°РІС‚РѕРјР°С‚РёС‡РµСЃРєРё
        -- Р’РѕР·РІСЂР°С‰Р°РµС‚ { Viewport, World, Camera } РґР»СЏ СЂР°Р±РѕС‚С‹ СЃРЅР°СЂСѓР¶Рё
        function Library:Viewport(properties)
            local Cfg = {
                Height   = properties.Height or 240;
                Items    = {};
            }

            local Items = Cfg.Items; do
                -- РћР±С‘СЂС‚РєР° вЂ” СЃРѕР±Р»СЋРґР°РµРј РѕС‚СЃС‚СѓРїС‹ РєР°Рє Сѓ РґСЂСѓРіРёС… РІРёРґР¶РµС‚РѕРІ (LayoutOrder РѕР±СЏР·Р°С‚РµР»РµРЅ РґР»СЏ UIListLayout)
                Items.Container = Library:Create("Frame", {
                    ...
                    BorderSizePixel  = 0;
                    BackgroundColor3 = rgb(8, 10, 18);
                    ClipsDescendants = true;
                })

                -- РўРѕРЅРєР°СЏ Р°РєС†РµРЅС‚РЅР°СЏ Р»РёРЅРёСЏ СЃРІРµСЂС…Сѓ (СЃС‚РёР»СЊ bbot)
                Library:Create("Frame", {
                    ...
                    BorderSizePixel  = 0;
                    BackgroundColor3 = themes.preset.accent;
                }); Library:Themify(Items.Container:FindFirstChild("\0") or Items.Container, "accent", "BackgroundColor3")
                ...
                Items.Viewport = Library:Create("ViewportFrame", {
                    ...
                    BackgroundColor3     = rgb(8, 10, 18);
                    BackgroundTransparency = 0;
                    Ambient              = rgb(155, 155, 155);
                    LightColor           = rgb(255, 255, 255);
                    LightDirection       = Vector3.new(-0.3, -1, -0.5);
                })

                Items.World = Library:Create("WorldModel", {
                    Parent = Items.Viewport;
                })

                local cam = Instance.new("Camera")
                cam.FieldOfView = 45
                cam.Parent = Items.Viewport
                Items.Viewport.CurrentCamera = cam
                Items.Camera = cam

                -- РћРІРµСЂР»РµР№ РґР»СЏ ESP-СЌР»РµРјРµРЅС‚РѕРІ РїРѕРІРµСЂС… РІСЊСЋРїРѕСЂС‚Р°
            ...
            Cfg.Viewport = Items.Viewport
            Cfg.World    = Items.World
            Cfg.Camera   = Items.Camera
            Cfg.Overlay  = Items.Overlay
        ...
        function Library:Configs(window) 
            ...
            Section:Button({Name = "Set Autoload", Callback = function()
                local selectedConfig = Library.Flags["config_Name_list"]
                if selectedConfig and selectedConfig ~= "" then
                    writefile(Library.Directory .. "/configs/autoload.txt", selectedConfig)
                    Notifications:Create({Name = "Autoload set: " .. selectedConfig})
                else
                    Notifications:Create({Name = "Select a config from the dropdown first!"})
                end
            end})

            if isfile(Library.Directory .. "/configs/autoload.txt") then
                local autoloadName = readfile(Library.Directory .. "/configs/autoload.txt")
                if autoloadName and autoloadName ~= "" and isfile(Library.Directory .. "/configs/" .. autoloadName .. ".cfg") then
                    Library:LoadConfig(readfile(Library.Directory .. "/configs/" .. autoloadName .. ".cfg"))
                    Library:UpdateConfigList()
                    Notifications:Create({Name = "Autoloaded Config (" .. autoloadName .. ")"})
                end
            end
